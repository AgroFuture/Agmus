...
func InitViperConfig() {
	...
	// App Ports
	pflag.String("app_port", "8085", "AGMUS server port")

	// Demo Mode
	pflag.Bool("demo_mode", true, "Switch for the demo mode. This will bypass auth check and use hardcoded token demo")

	// Persistence Config
	pflag.String("tania_persistence_engine", "sqlite", "AGMUS chidamli dvigatel. Mavjud dvigatellar: MySQL, SQLite, xotirada")

	// Persistence Config - SQLite
	pflag.String("sqlite_path", "agmus.db", "Path of sqlite file db")

	// Persistence Config - MySQL
	pflag.String("mysql_host", "127.0.0.1", "Mysql Host")
	pflag.String("mysql_port", "3306", "Mysql Port")
	pflag.String("mysql_dbname", "my_farm", "Mysql DBName")
	pflag.String("mysql_username", "developer", "Mysql username")
	pflag.String("mysql_password", "password", "Mysql password")
	...
	pflag.StringSlice("redirect_uri", []string{"http://localhost:8085/oauth2_implicit_callback"}, "URI for redirection after authorization server grants access token")
