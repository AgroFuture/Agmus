  ...
  .dump-crop-task
    ...
    .modal-body
      b-form(@submit.prevent="validateBeforeSubmit")
        ...
        .form-group
          label(for="quantity")
            translate Qancha o'simlikni chiqindiga ytashlamoqchisiz?
          vue-slider(v-model="task.quantity" v-bind:min="1" v-bind:max="max_value")
          span.help-block.text-danger(v-show="errors.has('quantity')") {{ errors.first('quantity') }}
        .form-group
          label(for="notes")
            translate Izohlar
          textarea.form-control#notes(type="text" :class="{'input': true, 'text-danger': errors.has('notes') }" placeholder="yig'imning-terimning ixtiyoriy izohlarini qoldiring" v-model="task.notes" name="notes" rows="2")
